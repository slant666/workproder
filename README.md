...
- 文件上传已限制大小和权限，目前使用后端持久化 Docker 卷保存附件；对象存储、病毒扫描和在线预览仍可继续增强。
- API 文档尚未自动生成，后续可接入 springdoc-openapi。
- 列表分页使用常规分页，极大数据量下可继续优化为游标分页或组合索引查询。
- 统计查询已有 Redis 缓存和失效处理，仍可继续优化缓存粒度和指标覆盖范围。
- 前端当前是单页应用原型，复杂权限和表单可以继续拆分组件。

## 当前已实现的扩展能力

- 邮件通知、站内通知和 WebSocket 实时通知。
- 工单 SLA、优先级规则、超时扫描和超时提醒。
- 多部门组织架构、RBAC 多角色和部门级数据权限。
- Redis 登录失败限流、统计缓存和缓存失效。
- GitHub Actions CI：自动运行前后端测试、构建、安全扫描、Compose 部署和健康检查。

## 仍可扩展方向

- 接入 OpenAPI/Swagger UI，自动生成接口文档。
- 将附件迁移到对象存储，并增加在线预览、病毒扫描和文件魔数校验。
- 增加审计日志查询页、筛选和导出功能。
- 补充独立的工单分类字典维护能力。
- 多实例部署时引入共享 Session 或改造为无状态认证。
